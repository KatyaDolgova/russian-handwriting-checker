...
class CheckService:
    ...
    async def stream_check(self, text: str, function_id: str) -> AsyncIterator[dict]:
        ...
        result = self._build_result(
            text, self._parse(full_text), full_text, fn_score_max, fn_min_words
        )
    ...
    def _build_result(
        ...
        data: dict,
        raw_text: str = "",
        fn_score_max: int | None = None,
        fn_min_words: int | None = None,
    ) -> dict:
        ...
        corrected = data.get("corrected") or (
            raw_text.strip() if is_generation else original_text
        )
        return {
