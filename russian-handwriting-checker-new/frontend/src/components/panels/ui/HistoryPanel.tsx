...
export const HistoryPanel = () => {
  ...
  const handleBulkMoveToFolder = async () => {
    ...
    await Promise.all(
      ids.map((id) => api.put(`/api/check/${id}`, { folder_id: fid }).catch(() => {})),
    );
  ...
  const handleBulkRemoveFromFolder = async () => {
    const ids = [...selectedIds];
    await Promise.all(
      ids.map((id) => api.put(`/api/check/${id}`, { folder_id: null }).catch(() => {})),
    );
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
            ...
            Папки
            {folders.length > 0 && (
              <span className="text-xs text-slate-400 font-normal">({folders.length})</span>
            )}
          </div>
          {showFolderMgr ? (
      ...
      <div className="flex items-center justify-between min-h-[1.25rem]">
        {filtered.length !== checks.length && (
          <p className="text-xs text-slate-400">
            Показано {filtered.length} из {checks.length}
          </p>
        )}
        {filtered.length > 0 && (
          <button
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((check) => {
            const folderName = check.folder_id
              ? folders.find((f) => f.id === check.folder_id)?.name
              : null;
            const displayTitle = check.title || check.filename;
            const textPreview = (check.original_text || check.corrected_text || '')
              .slice(0, 90)
              .trim();
                ...
                <div className="flex items-center">
                  ...
                  <div className="flex items-center gap-1 mr-3 shrink-0">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setConfirmDeleteId(isDeleting ? null : check.id);
                      }}
