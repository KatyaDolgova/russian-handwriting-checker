...
export const ProfilePage = () => {
  ...
  const changePassword = async () => {
    ...
    if (newPwd.length < 6) {
      setPwdMsg({ text: 'Минимум 6 символов', ok: false });
      return;
    }
    setSavingPwd(true);
    setPwdMsg(null);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="bg-white rounded-2xl border border-slate-100 p-6 flex items-center gap-5">
        ...
        <div>
          ...
          {data?.rank && (
            <span
              className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-medium border ${rankStyle.bg} ${rankStyle.text} ${rankStyle.border}`}
            >
              {data.rank.label}
            </span>
          )}
      ...
      {loading && <p className="text-sm text-slate-400 text-center py-4">Загрузка...</p>}
      {loadError && <p className="text-sm text-red-500 text-center py-4">{loadError}</p>}
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                <StatCard
                  ...
                  value={String(data.stats.total_checks)}
                />
                <StatCard
                  icon={<Users className="h-4 w-4 text-blue-400" />}
                  label="Уникальных учеников"
                  value={String(data.stats.unique_students)}
                />
                <StatCard
                ...
                <StatCard
                  ...
                  value={formatMonth(data.stats.most_active_month)}
                />
                <StatCard
                  icon={<FolderOpen className="h-4 w-4 text-amber-400" />}
                  label="Папок создано"
                  value={String(data.stats.total_folders)}
                />
                <StatCard
                  icon={<Users className="h-4 w-4 text-rose-400" />}
                  label="Групп создано"
                  value={String(data.stats.total_groups)}
                />
              </div>
